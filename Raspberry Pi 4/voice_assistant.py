...
class VoiceAssistant:
    def __init__(self, google_api_key, device_manager, firebase_handler):
    ...
    def execute_command(self, command):
        ...
        if action:
            ...
            if device_name:
                self.control_device(action, device_name)
            else:
    ...
    def control_device(self, action, device_name):
        device_id = next(device_id for device_id, name in self.device_manager.get_list_of_devices() if name == device_name)
        logger.info(f"Voice command to {action} {device_name} received")
